...
public class Model {
	...
	ArrayList<Particle> stickyArray = new ArrayList<Particle>(); //Our sticky area map.LinkedList
	...
	*/
	Model() {
		...
		for(int i=0; i<defaultV.particleAmount; i++) {
			...
			if(i<10){
				particle.color = defaultV.colorCode[i];
				particle.ColorName = defaultV.colorName[i];
				particle.isTracking = true;
			}
		}
	}
