	...
	*/
	protected HuffmanTree( BitFileReader bfr ) throws IOException {
		...
		if( bfr.readBit() ) {
			...
			left = null;
			right = null;
			value = bfr.readTwoBytes();         // Read an entire byte as the value.
			weight = 0;
      }
	}
	...
	*/
	public void saveToBuffer( BitFileWriter bfw ) throws IOException {
	  if( this.hasValue() ) {        	           // If it is a leaf (has a value)
			...
			boolean[] myTwoBytes = this.getValue();  // then write the Byte representing
			bfw.writeTwoBytes(myTwoBytes);              // the value, character.
	...
	*/
	...
	public String toString() {
		...
		for( Map.Entry<boolean[], Boolean[]> entry : dic.entrySet() ) {
			...
			ret += Arrays.toString(entry.getKey()) + ": " + (char)ByteDigit + ": ";
